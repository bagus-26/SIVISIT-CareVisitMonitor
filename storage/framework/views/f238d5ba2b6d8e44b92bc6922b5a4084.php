<?php $__env->startSection('title', 'Monitoring Lokasi Petugas'); ?>
    ...
    <div class="col-12 col-lg-4">
        <div class="sv-card sv-animate-in">
            ...
            <div class="lokasi-list" id="petugasList">
                ...
                <div class="lokasi-card p-3 border-bottom" data-id="<?php echo e($p->id); ?>"
                     data-lat="<?php echo e($p->latitude); ?>" data-lng="<?php echo e($p->longitude); ?>"
                     onclick="focusPetugas(this)">
                    <div class="d-flex align-items-center gap-3">
                        <div class="lokasi-avatar" style="background:<?php echo e($p->is_online ? '#34C759' : '#B0B0B0'); ?>;">
                            <?php echo e(strtoupper(substr($p->name, 0, 1))); ?>

                        </div>
                        <div style="flex:1;min-width:0;">
                            <div style="font-weight:600;font-size:13.5px;"><?php echo e($p->name); ?></div>
                            <div style="font-size:11.5px;color:var(--sv-text-muted);">
                                <i class="bi bi-geo-alt me-1"></i><?php echo e($p->location ?? '-'); ?>

                            </div>
                            <div style="font-size:11px;margin-top:2px;">
                                <?php if($p->latitude && $p->longitude): ?>
                                    <span class="<?php echo e($p->is_online ? 'petugas-online' : 'petugas-offline'); ?>">
                                        <i class="bi bi-circle-fill" style="font-size:7px;vertical-align:middle;"></i>
                                        <?php echo e($p->is_online ? 'Online' : 'Offline'); ?>

                                    </span>
                                    <span class="text-muted ms-2">
                                        <?php echo e($p->last_location_at ? $p->last_location_at->diffForHumans() : '-'); ?>
                            ...
                            <div style="font-size:11px;color:var(--sv-text-muted);margin-top:2px;">
                                <i class="bi bi-people me-1"></i><?php echo e($p->assigned_patients); ?> pasien
                                <span class="ms-2"><i class="bi bi-clipboard-check"></i> <?php echo e($p->today_visits); ?> kunjungan hari ini</span>
                            </div>
                        </div>
                        <a href="<?php echo e(route('admin.staff.edit', $p->id)); ?>" class="btn btn-sm btn-outline-secondary py-0" title="Edit Petugas">
...
<div id="patientData" data-patients='<?php echo json_encode($patients, 15, 512) ?>' style="display:none;"></div>
<?php $__env->stopSection(); ?>
...
function reassignPatient(patientId, newOfficerId) {
    ...
    fetch('/admin/patients/' + patientId + '/reassign', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content || '<?php echo e(csrf_token()); ?>' },
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sivisit_CareVisitMonitor\resources\views\admin\location-monitor.blade.php ENDPATH**/ ?>